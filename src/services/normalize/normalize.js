...
const normalizeItem = item => {
    ...
    const { dateCreated, description, nasaId, secondaryCreator, title, mediaType } = data;

    const link = camelizedItem.links[0];
    const { href: image } = link;

    return { dateCreated, description, nasaId, secondaryCreator, title, mediaType, image };
}
